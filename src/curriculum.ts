...
/**
 * Loads the one authoritative multi-track objective set from
 * content-packs/curriculum.json (generated by scripts/gen-curriculum.py from
 * each pack's manifest.json + index.json: sourceDocIds are derived mechanically
 * by matching each chunk's url against a chapter's docPath, while statements,
 * bloomLevel, and prerequisiteObjectiveIds are hand-authored per chapter in that
 * script, matching this file's own "hand-authored edges" rule - re-run the
 * script after editing a content pack's manifest.json or adding chapters).
 * Objective ids are namespaced per track (e.g. "rust-ch04-01-...", "firmware-how-to-build")
 * so a single CurriculumGraph safely spans every track at once - computeFrontier's
 * own track filter is what scopes a query back down to one track's view, not a
 * separate graph per track.
 */
export function loadCurriculumObjectives(curriculumJsonPath: string): CurriculumObjective[] {
  const data = JSON.parse(readFileSync(curriculumJsonPath, "utf-8")) as Record<string, CurriculumObjective[]>;
